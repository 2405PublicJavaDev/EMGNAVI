    ...
    <div className="w-full min-h-screen bg-white flex flex-col">
      <main className="flex-grow bg-white py-10">
        <div className="relative mx-auto top-[70px] bg-white rounded-lg p-8">
          ...
          <div className="relative w-full left-[30px] top-[30px] h-[80px] text-white text-3xl font-bold mb-8 flex">
            회원 신고 리스트
          </div>
          <table className="relative w-full border-collapse bg-white rounded">
            <thead>
              <tr className="bg-gray-100">
                <th className="border p-3 font-bold"></th>
                <th className="border p-3 font-bold">작성자 아이디</th>
                <th className="border p-3 font-bold">리뷰 내용</th>
                <th className="border p-3 font-bold">신고자 아이디</th>
                <th className="border p-3 font-bold">신고 날짜</th>
                <th className="border p-3 font-bold">신고 내용</th>
                <th className="border p-3 font-bold">신고 상태</th>
                <th className="border p-3 font-bold">정지 해제 날짜</th>
                <th className="border p-3 font-bold"></th>
              </tr>
            </thead>
            <tbody>
              {reportList.map((item, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className="w-[3%] border p-3 text-center">{item.no}</td>
                  <td className="w-[12%] border p-3 text-center">{item.writer}</td>
                  <td className="w-[23%] border p-3 text-center">{item.review}</td>
                  <td className="w-[12%] border p-3 text-center">{item.reporter}</td>
                  <td className="w-[10%] border p-3 text-center">{item.date}</td>
                  <td className="w-[10%] border p-3 text-center">{item.content}</td>
                  <td className="w-[10%] border p-3 text-center">{item.state}</td>
                  <td className="w-[10%] border p-3 text-center">{item.unfreezeDate}</td>
                  <td className="w-[10%] border p-3 text-center">
                    <button 
      ...
      >
        <div className="bg-white w-[40rem] max-h-[90vh] p-8 rounded-lg shadow-lg flex flex-col overflow-auto">
          ...
          {/* 확인 및 닫기 버튼 */}
          <div className="flex justify-end gap-4 mt-5 px-6">
