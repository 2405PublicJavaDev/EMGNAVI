...
const PharmacySearch = () => {
    ...
    const fetchFavorites = async () => {
        try {
            const response = await fetch('/api/pharmacy/favorites', { credentials: 'include' });
            if (response.ok) {
    ...
    const toggleFavorite = async (hpid, dutyName, dutyAddr, dutyTel1) => {
        ...
        try {
            if (favorites[hpid]) {
                await fetch(`/api/pharmacy/favorite?refNo=${hpid}`, { 
                    method: 'DELETE',
                    credentials: 'include'
                });
            } else {
                await fetch('/api/pharmacy/favorite', {
                    ...
                    body: new URLSearchParams({ refNo: hpid, dutyName, dutyAddr, dutyTel1 }),
                    credentials: 'include'
                });
            }
