...
const MedicineSearch = () => {
  ...
  return (
    <>
      ...
      <div className="w-full max-w-7xl mx-auto p-4 bg-white  relative top-16">
          {/* 검색 타이틀 */}
          ...
          <div className="flex justify-center mb-8">
            ...
            />
            <button className="bg-[#0b2d85] text-white px-4 h-[36px] text-[24px] rounded-r-md">
              검색
            </button>
          </div>

          {/* 의약품 테이블 */}
          <table className="table-auto w-full border-collapse border border-gray-300 shadow-lg rounded-lg text-center">
            <thead className="bg-[#cccccc1a]">
              <tr>
                <th className="border px-4 py-2">번호</th>
                <th className="border px-4 py-2">업체명</th>
                <th className="border px-4 py-2">제품명</th>
                <th className="border px-4 py-2">공개일자</th>
            ...
            <tbody>
              {dummyData.map((item, index) => (
                <tr key={index}>
                  <td className="border px-4 py-2">{item.번호}</td>
                  <td className="border px-4 py-2">{item.업체명}</td>
                  <td className="border px-4 py-2">{item.제품명}</td>
                  <td className="border px-4 py-2">{item.공개일자}</td>
