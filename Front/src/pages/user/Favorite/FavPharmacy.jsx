...
const generateDummyData = (count) => {
    return Array.from({ length: count }, (_, index) => ({
        dutyName: `압구정온누리약국 ${index + 1}`,
        dutyAddr: `${index} 서울특별시 강남구 압구정로 164, 아세아빌딩 105호 (신사동)`,
        dutyTel1: `${index + 1000}-${index + 1000}`,
    }));
};

export const FavPharmacy = () => {
    const [pharmacyList, setpharmacyList] = useState(generateDummyData(5));
    
    // 체크박스
    const [checkItems, setCheckItems] = useState([]);
    const allSelect = (checked) => {
        if(checked){
            const allList = pharmacyList.map((pharmacy) => pharmacy.dutyAddr);
    ...
    const selectpharmacy = openModalId !== null ? pharmacyList[openModalId] : null;
    // 상세 페이지로 이동
    const detailPage = () => {};
                        ...
                        <table className="relative w-full bg-white overflow-hidden">
                            <thead>
                                <tr className="bg-gray-100">
                                    ...
                                    <th className="p-3 font-bold">약국명</th>
                            ...
                            {pharmacyList.length === 0 ? 
                                ...
                                <tr className="h-[300px]">
                                    ...
                                    <div className="flex items-center justify-center h-full">
                                        <span className="font-bold text-lg">즐겨찾기 등록한 약국이 없습니다.</span>
                                    </div>
                                    </td>
                                </tr>
                                ) 
    ...
    >
        {selectpharmacy && (
            <div className="bg-white w-[40rem] max-h-[30vh] py-6 px-8 rounded-lg shadow-lg flex flex-col overclow-auto">
                <div className="pb-3">
                    <div className="font-bold">{selectpharmacy.dutyName} 을/를 삭제하시겠습니까?</div>
                </div>
