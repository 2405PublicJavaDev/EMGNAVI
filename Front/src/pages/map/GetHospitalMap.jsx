...
function GetHospitalMap() {
    ...
    useEffect(() => {
        if (searchRadius && latitude && longitude) {
            ...
            fetch(`/api/map/getAroundHospital?latitude=${latitude}&longitude=${longitude}&distance=${searchRadius}`)
                                    ...
                                    itemContent={(index, hospital) => (
                                        <div className="aed-group" style={{borderBottom: '1px solid #e5e7eb'}}>
                                            ...
                                            <TableRow key={`${index}-tel`}>
                                                <TableCell className="aed-tel text-sm text-gray-600" style={{ border: 'none', padding: '5px 10px' }}>{hospital.dutyTel1}</TableCell>
                                            </TableRow>
