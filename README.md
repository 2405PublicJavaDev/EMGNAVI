...
## 엄은지
  ### 좋았던 점(Keep)
  - 프론트엔드 작업의 효율성과 디자인: Tailwind CSS로 피그마 디자인을 빠르게 구현해 효율적이고 예쁜 결과물을 얻었습니다.
  - 회원관리 기능 목표 달성: 회원관리 기능을 맡아 계획했던 모든 기능을 성공적으로 구현했습니다.
  - 리액트 지식 습득: 프로젝트를 통해 useAxios, Tailwind, props 사용법을 익혔습니다.
  ### 아쉬웠던 점(Problem)
  - 새로운 지식 습득의 어려움: 처음 접한 Tailwind CSS와 리액트로 인해 겪었던 기술적 어려움.
  - 소셜 로그인 구현의 좌절: 특정 기능 구현에서 계속 실패하면서 겪었던 정신적 부담.
  - 리팩토링을 못한 아쉬움: 시간과 능력 부족으로 더 나은 코드를 작성하지 못한 아쉬움.
  ### 기대되는 점(Try)
  - 리액트 코드를 타임리프로 변환
  - 스프링 시큐리티와 OAuth 적용
